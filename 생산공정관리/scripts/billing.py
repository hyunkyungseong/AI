...
def calc_공급가맵(df_all, 단가맵, 자재map, 의뢰서번호셋):
    ...
    for _, _r in _g.iterrows():
        ...
        # 봉투제작비(자재비)는 이 리스트 요약용 함수에선 용지제작비 등 다른 항목과 동일하게 단순
        # 곱셈만 쓴다(자재코드별 등록 단가는 build_품목행()에서만 반영 — 2026-08-15 출력자재비·삽지비
        # 이관 때와 동일한 기존 결정). 각대대봉투단가는 더 이상 계산에 쓰지 않는다(2026-08-17,
        # "각대봉투"도 그냥 봉투제작단가 밑 자재단가 하나로 등록하는 방식으로 통합 — 상세:
        # `.claude/plans/plan_단가마스터_자재명정규화.md`).
        봉투제작비_금액 = (일반봉투 + 각대대봉투) * rates.get("봉투제작단가", 0)
        항목금액 = {
...
def build_품목행(df_all, 단가맵, 자재map, 의뢰서번호셋):
    ...
    for _, _r in _g.iterrows():
        ...
        if 청구페이지있음:
            _자재비례배분_처리(행데이터, "출력비", 작업, rates, 용지자재별, 청구, rates.get("출력단가", 0))
        elif 용지 > 0:
        ...
        elif rates.get("출력단가", 0) > 0 and 청구 > 0:
            # 용지 자재이력 자체가 없으면(휴먼 에러로 누락) 기존처럼 출력페이지 합계로 청구(폴백) —
            # 불일치 안내는 자재수량_불일치_목록()이 별도로 띄운다.
            행데이터[("출력비", 작업, rates.get("출력단가", 0), None)]["수량"] += 청구
            행데이터[("출력비", 작업, rates.get("출력단가", 0), None)]["금액"] += 청구 * rates.get("출력단가", 0)
