...
| SKILL-45 | HeidiSQL 등 DB GUI 툴의 테이블 목록 패널에 뜨는 "행 수"는 InnoDB 통계 샘플링 기반 근사치 — 로컬·사무실 PC 데이터 건수를 정확히 비교할 땐 반드시 `SELECT COUNT(*)`로 직접 조회할 것 | ✅ 2026-08-24 |
| SKILL-46 | 자동완성 검색 콤보박스는 클릭/포커스 시 "전체 목록"이 아니라 "현재 입력값 기준 필터"로 열 것 — 값이 비어있으면 필터가 자연히 전체를 보여주므로 "기존 값 있으면 클릭해도 안 열림" 문제도 같이 해결되고, "검색해서 좁혀놓은 뒤 재클릭하면 필터가 통째로 풀리는" 문제가 안 생김. 위치·높이(위/아래 자동반전 + 남은 공간만큼 가변)는 `frontend/lib/dropdownPosition.ts`(2026-08-16 최초 작성) 공용 함수를 재사용할 것 — 호출부마다 복사해두면 한쪽만 고치고 놓치기 쉬움 | ✅ 2026-08-24 |
...
## SKILL-45. HeidiSQL 테이블 목록 "행 수"는 InnoDB 근사치

**목적:** 로컬 PC·사무실 PC 등 두 DB의 데이터 건수를 비교할 때, GUI 툴 화면만 보고 "차이가 있다"고
오판하지 않기

**검증 상태:** ✅ 완료 (2026-08-24, 자재사용현황 배포 후 로컬 12,571 vs 사무실 12,818로 다르게
보인다는 제보 — 실제로는 둘 다 동일했음)

**문제 상황:**
- HeidiSQL(및 비슷한 DB GUI 툴)의 왼쪽 테이블 목록 패널에 뜨는 "행 수"는 `SHOW TABLE STATUS`가
  반환하는 InnoDB 통계 기반 **근사치**다 — MyISAM과 달리 InnoDB는 정확한 행 수를 별도로 유지하지
  않고 내부 샘플링으로 추정하기 때문에, 실제 `COUNT(*)`과 수백 건까지도 차이가 날 수 있다
- 로컬·사무실 PC 양쪽 다 같은 파일·같은 코드로 재적재했는데도 이 패널 숫자만 보고 비교하면 "왜
  다르지?"라는 잘못된 결론에 이르기 쉽다 — 실제로는 둘 다 똑같았음(12,717건)

**핵심 규칙:**
```sql
-- 근사치만 보여줌 (HeidiSQL 테이블 목록 패널이 내부적으로 쓰는 것과 동일)
SHOW TABLE STATUS LIKE '자재사용현황';

-- 정확한 값 — 두 DB를 비교할 땐 반드시 이걸로
SELECT COUNT(*) FROM 자재사용현황;
```

**주의사항:**
- TRUNCATE 직후에도 이 패널 값이 즉시 갱신 안 되고 예전 캐시값을 잠깐 보여줄 수 있음 — 새로고침(F5)
  해도 여전히 근사치라는 점은 동일
- 자재종류별·날짜범위별 세부 비교가 필요하면 `GROUP BY`·`MIN/MAX` 등을 곁들인 `COUNT(*)` 쿼리로
  어느 부분에서 차이가 나는지 먼저 좁혀본 뒤에, 그래도 실제로 다르면 그때 파일·코드 버전 차이를
  의심할 것

---

## SKILL-46. 자동완성 검색 콤보박스 — 클릭 시 "현재 입력값 기준 필터"로 열기 + 위치 계산 공용화

**목적:** 자유 입력+후보 선택을 겸하는 콤보박스(`EditableCombo.tsx` 등)에서, 검색해서 좁혀둔 상태로
재클릭해도 필터가 안 풀리고, 화면 아래쪽에서 열어도 목록이 잘리거나 너무 짧지 않게 하기

**검증 상태:** ✅ 완료 (2026-08-24, 거래처마스터 담당자관리 화면 실사용 제보로 발견·수정)

**문제 상황:**
- `EditableCombo.tsx`는 2026-07-19에 "이미 값이 선택된 상태에서 클릭하면 전체 후보가 다시 뜨지
  않고 지워야만 목록이 보이는" 네이티브 `<datalist>` 문제를 고치려고, **클릭/포커스 시 입력값과
  무관하게 항상 전체 후보(`search=null`)를 보여주도록** 만들어뒀었음
- 그런데 이 설계가 새 문제를 만듦 — "현대해상"까지 타이핑해서 목록이 정확히 좁혀진 상태에서 화면을
  스크롤하거나 입력칸을 다시 클릭하기만 해도(흔한 조작) `search`가 `null`로 리셋되며 필터가 통째로
  풀려, 방금 타이핑한 글자와 무관한 전체 목록이 다시 나타남

**핵심 규칙:**
```tsx
// 나쁜 예 — 클릭/포커스 시 항상 필터를 초기화(전체 표시)
...
// 좋은 예 — 클릭/포커스 시 "현재 입력값"으로 필터를 채운다
// (비어있으면 filter()가 자연히 전체를 보여줘 원래 목적도 그대로 달성됨)
function openFullList(el: HTMLInputElement) {
  setOpen(true);
  setSearch(el.value);
  el.select(); // 전체 선택돼 있어 바로 타이핑하면 처음부터 다시 필터링됨
}
```

**위치·높이 — 공용 함수로 추출:**
```ts
// frontend/lib/dropdownPosition.ts (2026-08-16 PricingMaterialSection.tsx에서 최초 작성,
// 2026-08-24 EditableCombo.tsx도 재사용하며 공용화)
export function 계산_드롭다운위치(el: HTMLElement, 기본최대높이 = 192): DropdownPos {
  const r = el.getBoundingClientRect();
  const 아래공간 = window.innerHeight - r.bottom - 8;
  const 위공간 = r.top - 8;
  const 아래로 = 아래공간 >= 기본최대높이 || 아래공간 >= 위공간;
  const maxHeight = Math.max(80, Math.min(기본최대높이, 아래로 ? 아래공간 : 위공간));
  return 아래로
...
```
- 호출부는 `createPortal(<ul style={{position:"fixed", ...}}>, document.body)`로 그려야
  스크롤 컨테이너 경계에 안 잘림(SKILL-38 참고)
- 목록을 "훑어봐야" 하는 화면(이름을 모르고 찾는 경우)이면 기본최대높이를 192보다 크게(예: 280)
  넘겨서 한 번에 더 많이 보이게 할 수 있음 — 두 번째 인자로 조절

**주의사항:**
- "바깥 클릭 감지"도 포털로 그린 목록은 입력칸의 DOM 하위가 아니므로, 입력칸 ref 하나만으로는
  안 되고 목록 자체의 ref도 같이 확인해야 한다(`inputRef.contains(e.target) || listRef.contains(e.target)`
  둘 다 아니어야 진짜 바깥 클릭)
- 같은 로직을 두 곳에 복사해두면 한쪽만 고치고 다른 쪽을 놓치기 쉬우므로(SKILL-42류 함정), 새로
  비슷한 콤보박스가 필요하면 이 공용 함수부터 재사용할 것
