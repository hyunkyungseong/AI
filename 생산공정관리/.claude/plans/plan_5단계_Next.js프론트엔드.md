...
- **거래명세서 화면 미리보기 기능 (2026-07-19 요청, 보류):** 미발행 목록에서 "거래명세서 요청" 시 Excel 다운로드 대신 화면에서 바로 보는 미리보기 요청 있었으나, [4단계] 끝난 뒤 재검토하기로 함(이제 재검토 시점 — 새 세션에서 필요 시 진행). 조사해둔 참고 사항:
  - `scripts/billing.py`의 `generate_거래명세서_excel()`(101~347행) 안에 품목 행 계산 로직(111~171행, `행데이터`/`정렬행`)이 Excel 셀 기록과 뒤섞여 있음 — 미리보기를 만들려면 계산 부분만 별도 함수(예: `build_품목행()`)로 분리해서 Excel 생성 함수와 신규 JSON 미리보기 API가 공유하도록 리팩터링 필요
  - 기존 `GET /거래명세서엑셀/{no}`는 이미 채번된 건만 대상 — "요청 전 미리보기"를 하려면 채번 전 선택 상태에서도 계산 가능한 별도 경로 필요
  - 결정 필요: ① 미리보기를 "요청 클릭→미리보기→확정" 흐름으로 할지 "요청 완료 후 결과 확인용"으로 할지 ② Excel과 시각적으로 동일하게(직인 포함) 만들지, 항목·합계만 보여주는 단순 표로 할지
