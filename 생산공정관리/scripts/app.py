...
with tab4:
    ...
    # 계산·Excel 생성 본체는 scripts/billing.py로 이전(api.py와 공유) — 여기서는 이 탭에서 만든
    # df_all·_t4_단가맵·_t4_자재map을 고정해서 넘겨주는 얇은 래퍼만 유지 (호출부·동작 완전히 동일)
    def calc_공급가맵(의뢰서번호셋):
        return billing.calc_공급가맵(df_all, _t4_단가맵, _t4_자재map, 의뢰서번호셋)

    def generate_거래명세서_excel(의뢰서번호셋, 발행일):
        return billing.generate_거래명세서_excel(df_all, _t4_단가맵, _t4_자재map, 의뢰서번호셋, 발행일)
    ...
    with t4b:
        ...
        if t4b_탭 == "기본정보":
            ...
            if not 거래처목록_단가:
                ...
                # 2026-08-15 — 같은 (거래처+업무명+작업명)이라도 실제 사용된 자재(용지·봉투 종류 등)에
                # 따라 단가가 달라야 하는 업무 지원("단가마스터 자재명 정규화")은 이 화면(Streamlit)에는
                # 이식하지 않기로 확정 — 최종 배포 목표가 이미 Next.js로 정해져 있어, 이 기능부터는
                # 새 웹 화면(거래처 마스터 → 단가관리)만 지원한다. 여기서는 지금까지처럼 항목당 단가
                # 1개(기본단가)만 등록·수정할 수 있다.
                st.info(
                    "📌 같은 작업이라도 자재(용지·봉투 종류 등)별로 단가를 다르게 등록하려면 "
                    "새 웹 화면(거래처 마스터 → 단가관리)을 사용해 주세요. 이 화면에서는 항목당 "
                    "기본단가 1개만 등록·수정됩니다."
                )
                단가df = load_단가마스터()
