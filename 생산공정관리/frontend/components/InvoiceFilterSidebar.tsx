...
import type { useInvoiceFilters } from "@/lib/useInvoiceFilters";

type Props = {
  filters: ReturnType<typeof useInvoiceFilters>;
};

// FilterSidebar.tsx와 동일한 접기/펼치기 패턴·필터 종류(사업부→조회기간→담당자→거래처→업무명) —
// 탭4 "미발행 목록" 전용. 조회기간 기본값만 탭1~3과 달리 전체 기간(제한 없음, useInvoiceFilters 참고).
export default function InvoiceFilterSidebar({ filters }: Props) {
