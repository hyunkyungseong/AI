...
def merge_자재(df, 자재df):
    """
    이미 로드된 자재df(컬럼: 업무의뢰서번호·작업내역서번호·작업일자·자재종류·자재명·사용량, 앞 2개는 int, 작업일자는 YYYY-MM-DD 문자열)를
    ...
    if "자재명" in 자재df.columns:
        자재df.loc[봉투_mask, "자재형태"] = 자재df.loc[봉투_mask, "자재명"].apply(_봉투종류)
    else:
        # 자재명 컬럼이 없는 호출부(예: 자재명을 안 보내는 실시간 API)는 구분 불가 → 계산 시 일반봉투로 간주됨
        자재df.loc[봉투_mask, "자재형태"] = "일반봉투"

    long_grouped = (
        자재df.groupby(["업무의뢰서번호", "작업내역서번호", "작업일자", "자재종류", "자재형태"], dropna=False)["사용량"]
        .sum()
        .reset_index()
    )
...
def 자재_행_변환(row):
    ...
    return (
        str(int(row["업무의뢰서번호"])), int(row["작업내역서번호"]),
        row["작업일자"], row["자재종류"], _문자(row.get("자재형태")), int(row["사용량"]),
    )
