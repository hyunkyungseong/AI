...
def _자재별_처리(행데이터, 품목, 작업, rates, 자재별dict, 총수량, 기본단가):
...
def _자재비례배분_처리(행데이터, 품목, 작업, rates, 자재별dict, 총량, 기본단가):
...
def _작업별_품목누적(행데이터, 작업, rates, *, 청구페이지있음, 청구페이지, 봉입건수, 장수,
    ...
    # 계산된 값(아무도 검증 안 한 원본)이면, 더 신뢰되는 용지 자재사용량 × 인쇄면 배율(단면=1배/
    # 양면=2배, 단가마스터.인쇄면)로 대체한다 — 상세: `.claude/plans/plan_단가마스터_자재명정규화.md`.
    if 청구페이지있음:
        _자재비례배분_처리(행데이터, "출력비", 작업, rates, 용지자재별, 청구페이지, rates.get("출력단가", 0))
    elif 용지 > 0:
        배율 = 2 if rates.get("인쇄면") == "양면" else 1
        용지자재별_배율 = {k: v * 배율 for k, v in 용지자재별.items()}
        _자재별_처리(행데이터, "출력비", 작업, rates, 용지자재별_배율, 용지 * 배율, rates.get("출력단가", 0))
    elif rates.get("출력단가", 0) > 0 and 청구페이지 > 0:
        # 용지 자재이력 자체가 없으면(휴먼 에러로 누락) 기존처럼 출력페이지 합계로 청구(폴백) —
        # 불일치 안내는 자재수량_불일치_목록()이 별도로 띄운다.
        행데이터[("출력비", 작업, rates.get("출력단가", 0), None)]["수량"] += 청구페이지
        행데이터[("출력비", 작업, rates.get("출력단가", 0), None)]["금액"] += 청구페이지 * rates.get("출력단가", 0)
    ...
    _자재별_처리(행데이터, "봉입비", 작업, rates, 봉투자재별 if 봉투합 > 0 else {}, 봉입_기준수량, rates.get("봉입단가", 0))
    ...
    _자재별_처리(행데이터, "출력자재비", 작업, rates, 용지자재별, 용지, rates.get("용지제작단가", 0))
    _자재별_처리(행데이터, "봉입자재비", 작업, rates, 봉투자재별, 봉투합, rates.get("봉투제작단가", 0))
    _자재별_처리(행데이터, "삽지비", 작업, rates, 삽지자재별, 삽지, rates.get("삽지제작단가", 0))
...
def build_단가맵(단가df, 자재단가df=None):
        ...
        (r["거래처명"],
         ...
         None if pd.isna(r["작업명"]) else r["작업명"]): {
            **{c: float(r[c] or 0) for c in 단가컬럼},
            # 부가세구분·인쇄면은 숫자가 아니라 문자열이라 단가컬럼과 분리 처리(부가세구분 2026-07-28,
            # 인쇄면 2026-08-17 — 청구페이지 원본이 없어 출력비를 용지 자재사용량으로 대체 계산할 때
            # 몇 배로 환산할지 결정하는 값, `_r["인쇄면"]` 참고).
