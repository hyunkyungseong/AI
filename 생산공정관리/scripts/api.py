...
def _공정단가df_조회(cur):
    """billing.build_단가맵()의 공정단가df 인자로 바로 쓸 수 있는 형태(거래처명·업무명·작업명·공정코드·단가·비고)로
    단가마스터_공정단가를 단가마스터와 조인해서 반환(2026-08-21, 공정별 단가 청구 —
    `.claude/plans/plan_공정별단가청구.md`). 자재단가와 달리 공정은 고정 8종 enum이라 매칭 테이블 조인이
    필요 없다. 비고(2026-08-24)는 거래명세서 미리보기 원본 표에 표시하기 위함 —
    `.claude/plans/plan_공정단가_비고_미리보기표시.md`."""
    cur.execute("""
        SELECT dm.거래처명, dm.업무명, dm.작업명, gd.공정코드, gd.단가, gd.비고
        FROM 단가마스터_공정단가 gd
...
def _거래명세서_엑셀_시트목록(cur, 거래명세서번호):
    ...
    cur.execute("SELECT 사업자등록번호, 수신이메일, 조별표지 FROM 거래처마스터 WHERE 거래처명=%s", (거래명세서["거래처명"],))
    _거래처행 = cur.fetchone()
    # 표지("요청내용") 시트용 정보(2026-08-29) — 거래명세서.품목/공급가액/세액/합계는 확정 시점에
    # 이미 화면 표시값(직접입력 override 반영됨)으로 저장돼 있으므로 재계산 없이 그대로 쓴다.
    # 조별 부분합(그룹표시_공급가액 등)과 혼동하지 말 것 — 표지는 항상 이 인보이스 전체 총액이다.
    # "담당자이메일"은 내부 계정 담당자(B31에 쓰이는 담당자 변수)가 아니라 거래처마스터에 등록된
    # 거래처 수신이메일이다(2026-08-29, 사용자 확인 — 표지는 "그 거래처로 보내는 요청" 개념이므로
    # 수신처 이메일을 보여줘야 함).
    표지정보 = {
        "거래명세서번호": 거래명세서번호,
        "거래처명": 거래명세서["거래처명"],
        "사업자등록번호": _거래처행["사업자등록번호"] if _거래처행 else None,
        "발행일": 발행일,
        "품목": 거래명세서["품목"],
        "공급가액": 거래명세서["공급가액"],
        "세액": 거래명세서["세액"],
        "합계": 거래명세서["합계"],
        "수신이메일": _거래처행["수신이메일"] if _거래처행 else None,
    }

    cur.execute(
        "SELECT 조, 코드, 품목, 수량, 단가, 금액, 조상단업무명, 구분표시, 규격, 비고, 구분자 FROM 거래명세서_품목 "
        "WHERE 거래명세서번호=%s AND 구분='최종' ORDER BY 순서",
        (거래명세서번호,),
    )
    저장된_최종 = cur.fetchall()
    ...
    for idx, 조 in enumerate(그룹순서):
        ...
        # 조상단업무명(2026-08-30) — 이 조에 사용자가 직접 입력한 상단 업무명이 있으면 그 값을,
        # 없으면 기존처럼 인보이스 공유 업무명을 그대로 B12에 쓴다.
        효과_업무명 = next((r["조상단업무명"] for r in 그룹맵[조] if r.get("조상단업무명")), None) or 업무명
        바이트 = billing.write_거래명세서_excel(
            품목행목록, 그룹표시_공급가액, 그룹표시_세액, 거래명세서["거래처명"], 효과_업무명, 발행일, 담당자
        )
        시트명 = 조 or 기본시트명
        시트_목록.append((바이트, 시트명))
        # 조별표지(2026-08-30)용 — 부분합은 위에서 이미 계산된 값을 그대로 모아둠(새 계산 없음).
        # "품목"은 이 조 시트의 B12(write_거래명세서_excel()이 실제로 써넣은 업무명)를 그대로
        # 읽어와서 쓴다 — 라인아이템(표시품명)을 다시 조합하지 않아, 업무명 계산 로직이 바뀌어도
        # 표지가 항상 실제 시트 내용과 일치한다.
        조별표지_목록.append({
            "번호": idx + 1,
            "시트명": 시트명,
            "품목": billing.읽기_시트2_셀(바이트, "B12"),
            "공급가액": 그룹표시_공급가액,
            "세액": 그룹표시_세액,
            "합계": round(그룹표시_공급가액 + 그룹표시_세액),
        })

    if _거래처행 and _거래처행.get("조별표지") and len(그룹순서) > 1:
        표지정보["조별목록"] = 조별표지_목록

    if len(그룹순서) > 1 and 거래명세서.get("통합시트명") and not (_거래처행 and _거래처행.get("조별표지")):
        ...
        # 표지("요청내용") "품목"도 조별표지와 동일한 원칙 — 확정 시점에 저장된 구식 업무명 조합
        # (거래명세서.품목) 대신, 방금 만든 통합 시트에 실제로 적힌 B12 값을 그대로 읽어와서 쓴다
        # (2026-08-30, 사용자 제보 — BC카드 등 통합시트 케이스에서 품목이 안 맞던 문제).
        표지정보["품목"] = billing.읽기_시트2_셀(통합바이트, "B12")
...
def 거래명세서엑셀(no: str):
    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            ...
            표지정보 = None
            if 묶음번호:
                # 레거시 하위호환 경로 — 서로 다른 거래명세서 여러 건을 합치는 것이라 표지 하나로
                # 대표할 수 없으므로 표지정보를 만들지 않는다(기존과 동일하게 표지 없이 합쳐짐).
                cur.execute(
                ...
                시트_목록, 표지정보 = _거래명세서_엑셀_시트목록(cur, no)

    엑셀바이트 = billing.combine_거래명세서_시트들(시트_목록, 표지정보)
    if 엑셀바이트 is None:
...
@app.get("/조별상단업무명기본값", dependencies=인증필요)
def 조별상단업무명기본값(거래처명: str = Query(...), 업무명_목록: List[str] = Query(default=[])) -> Dict[str, str]:
    """개별 조 시트 B12에 표시할 조별 상단 업무명 입력칸 기본값(2026-08-30) — 이 거래처+업무명조합의
    조마다 마지막에 저장했던 값을 한 번에 `{조: 조상단업무명}` 형태로 반환한다(조가 여러 개라
    프론트가 그룹 헤더마다 따로 조회하지 않고 한 번의 요청으로 전부 프리필). `통합시트기본값()`과
    동일한 이유로 업무명조합 키는 항상 서버가 재계산한다."""
    업무명조합 = billing.업무명조합_키(업무명_목록)
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT 조, 조상단업무명 FROM 조상단업무명설정 WHERE 거래처명=%s AND 업무명조합=%s",
                (거래처명, 업무명조합),
            )
            rows = cur.fetchall()
    return {r["조"]: r["조상단업무명"] for r in rows if r["조상단업무명"]}
...
def 거래명세서미리보기(요청: 거래명세서미리보기_요청):
    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            ...
            자재df = pd.DataFrame(자재행) if 자재행 else pd.DataFrame(columns=["업무의뢰서번호", "작업이름", "자재종류", "자재형태", "자재코드", "자재명", "사용량"])
            자재단가df = pd.DataFrame(자재단가행) if 자재단가행 else pd.DataFrame(columns=["거래처명", "업무명", "작업명", "품목", "단가", "자재단가_id", "표시명", "인쇄면", "자재코드", "자재명"])
            공정단가df = pd.DataFrame(공정단가행) if 공정단가행 else pd.DataFrame(columns=["거래처명", "업무명", "작업명", "공정코드", "단가", "비고"])
            ...
            단가미등록 = []
            # 공정비고(2026-08-24): 공정단가 등록 시 입력한 비고를 원본 표에 표시하기 위한 곁가지
            # 수집 — 단가미등록과 동일한 패턴(계산 결과에는 영향 없음). 상세:
            # `.claude/plans/plan_공정단가_비고_미리보기표시.md`.
            공정비고맵 = {}
            ...
            원본목록 = billing.정렬행_원본목록(정렬행, 코드맵, 공정비고맵)
            ...
            # 조별표지(2026-08-30) — 이 거래처가 통합 명세서를 안 쓰고 조별로 표지를 나눠 보여주는
            # 대상인지, 화면이 "통합시트명" 입력칸을 감출 수 있도록 함께 내려준다.
            cur.execute("SELECT 조별표지 FROM 거래처마스터 WHERE 거래처명=%s", (거래처명,))
            _조별표지행 = cur.fetchone()
            조별표지_설정 = bool(_조별표지행 and _조별표지행["조별표지"])
    ...
    return {
        ...
        "단가미등록": 단가미등록,
        "품목": [
            {"코드": row["코드"], "품목": row["품목"], "작업명": row["작업명"], "업무명": row.get("업무명"),
             "자재명": row.get("자재명"), "공정비고": row.get("공정비고"),
...
class 거래처행(BaseModel):
    ...
    # 조별표지(2026-08-30) — 신규 등록 시 기본값은 항상 비워둠(False), 역발행과 동일 원칙.
    조별표지: Optional[bool] = False
...
def 거래처마스터_추가(거래처: 거래처행):
    ...
    try:
        with get_db() as conn:
            with conn.cursor() as cur:
                cur.execute("""
                    INSERT INTO 거래처마스터 (거래처명, 사업자등록번호, 수신이메일, 비고, 역발행, 조별표지, 등록일, 수정일)
                    VALUES (%s,%s,%s,%s,%s,%s,%s,%s)
                """, (거래처명, 거래처.사업자등록번호, 거래처.수신이메일, 거래처.비고,
                      1 if 거래처.역발행 else 0, 1 if 거래처.조별표지 else 0,
...
def 거래처마스터_수정_요청(name: str, 거래처: 거래처행_수정):
    """거래처 1건 수정 — 사업자등록번호·수신이메일·비고·역발행·조별표지만 변경 가능, 거래처명은
    요청 바디에 필드 자체가 없어 API 레벨에서부터 변경 불가."""
    from datetime import date
    오늘 = str(date.today())
    with get_db() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                UPDATE 거래처마스터
                SET 사업자등록번호=%s, 수신이메일=%s, 비고=%s, 역발행=%s, 조별표지=%s, 수정일=%s
                WHERE 거래처명=%s
            """, (거래처.사업자등록번호, 거래처.수신이메일, 거래처.비고,
                  1 if 거래처.역발행 else 0, 1 if 거래처.조별표지 else 0, 오늘, name))
            if cur.rowcount == 0:
                ...
                cur.execute("""
                    INSERT INTO 단가마스터
                        (거래처명, 업무명, 작업명, 출력단가, 봉입단가, 추가봉입단가, 동봉물삽입단가,
                         용지제작단가, 봉투제작단가, 삽지제작단가, 용지제작무상, 봉투제작무상, 삽지제작무상,
                         각대대봉투단가, 각대대봉투봉입단가,
                         부가세구분, 인쇄면, 청구단위, 비고, 등록일, 수정일)
                    VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
                """, (단가.거래처명, 단가.업무명, 단가.작업명, 단가.출력단가, 단가.봉입단가, 단가.추가봉입단가, 단가.동봉물삽입단가,
                      단가.용지제작단가, 단가.봉투제작단가, 단가.삽지제작단가, 단가.용지제작무상, 단가.봉투제작무상, 단가.삽지제작무상,
                      단가.각대대봉투단가, 단가.각대대봉투봉입단가,
...
class 품목행_입력(BaseModel):
    ...
    # 삽입 행("새 행 추가"의 변형). write_거래명세서_excel()이 이 값이 true인 행은 품명~금액 영역을
    # 하나로 병합해 캡션 텍스트만 채운다 — 상세: `.claude/plans/bb-ab-vast-piglet.md`.
    구분자: Optional[bool] = None
...
class 거래명세서요청_요청(BaseModel):
    ...
    # 개별 조 시트 B12에 표시할 조별 상단 업무명(2026-08-30) — {조: 조상단업무명} 형태. 값이 없는
    # 조는 기존처럼 인보이스 공유 업무명을 그대로 씀(하위호환, `통합시트명`/`상단업무명`과는 독립적).
    조상단업무명맵: Optional[Dict[str, str]] = None
...
def 거래명세서요청(요청: 거래명세서요청_요청, 사용자: str = Depends(auth.get_current_user)):
    ...
    # 이번 확정에서 규칙을 새로 만들거나 고쳤으면(요청.규칙 — 프론트가 매 확정마다 오른쪽 표의
    # 조건식 있는 행 전체를 보내므로 이 화면을 쓰는 한 사실상 항상 채워짐) 그 규칙을 기준선 계산에도
    # 즉시 반영한다. 저장 로직(_규칙_저장/_통합규칙_저장, 아래)이 항상 "전체 교체" 방식이라 요청.규칙은
    # 이번 확정 이후 실제로 DB에 남을 규칙 전체와 동일 — 위 _통합조건식_판정()이 조회해온 규칙목록은
    # "이번 확정 이전" 상태라 방금 만든 규칙이 아직 안 잡혀 있다. 이걸 그대로 기준선으로 쓰면(2026-08-31,
    # D-202608-00133 사용자 제보로 발견) "신규 규칙 생성 + 즉시 확정" 케이스에서 기준선이 옛(비어있는)
    # 규칙 상태로 남아, 새 규칙 자신이 만든 이름 변경까지 전부 품목삭제+품목추가 잡음으로 뒤덮이고 — 그
    # 아래 아래 guard(품목이력_항목 계산, 기준_대비_변경 and 효과_규칙목록)가 그 잡음을 걸러내려다 그 안에
    # 섞인 진짜 수량·단가 수정(수량 130→906)까지 통째로 사라졌었다.
    효과_규칙목록 = [r.model_dump() for r in 요청.규칙] if 요청.규칙 is not None else 규칙목록

    if 효과_규칙목록:
        규칙적용결과, 미분류 = billing.적용_규칙(원본목록, 효과_규칙목록)
    ...
    최종목록 = [
        {"코드": r.코드 or "", "표시품명": r.품목, "수량": r.수량, "단가": r.단가,
         ...
         "수동입력": bool(r.수동입력), "구분자": bool(r.구분자)}
    ...
    # 프론트(Tab4Invoice.tsx)는 제출 직전 Math.round()로 항상 원 단위 정수로 반올림해서 보내는데,
    # "별도" 거래처는 부가세_표시분리()가 공급가액을 그대로(반올림 없이) 반환해 소수 단가(예: 16.8원)
    # 품목이 섞이면 0.2원 같은 잔여가 남는다 — 기준선도 원 단위로 맞춰야 실제로는 아무것도 안 바뀐
    # 확정 건에서 "공급가액"·"합계" 수정이력이 잘못 남고 "편집됨" 배지가 붙는 오탐을 막는다
    # (2026-09-01, D-N-202609-00001 제보로 발견 — 업무명 분리 작업과는 무관한 기존 버그).
    기준_공급가액, 기준_세액 = round(기준_공급가액), round(기준_세액)
    ...
    # 효과_규칙목록이 하나도 없으면(개별·통합조건식 통틀어 이번 확정에 쓰인 규칙이 전무) 기준선이
    # 원본목록 그대로라 품목 비교 자체가 무의미함 — 조건식을 새로 만들면서 동시에 확정한 경우 새로
    # 묶인 항목이 전부 "품목삭제+품목추가" 스푸리어스 기록으로 남는 문제(2026-08-17, D-202608-00079
    # 조사 중 발견)를 막기 위한 guard였다. **2026-08-31 수정**: 원래는 "규칙목록"(이번 확정 이전의
    # 스테일한 DB 상태)을 봤는데, 그러면 "신규 규칙 생성 + 즉시 확정" 케이스에서 기준목록이 그
    # 스테일한 상태로 계산돼(위 효과_규칙목록 도입 전) 새 규칙 자신이 만든 이름 변경까지 전부
    # 잡음으로 잡히고, 그 guard가 통째로 걸러내면서 그 안에 섞인 진짜 수량·단가 수정까지 함께
    # 사라졌었다(D-202608-00133 제보 — 수량을 130→906으로 고쳤는데 공급가액·부가세·합계만 남고
    # 수량 변경 자체가 기록 안 됨). 이제 기준목록 자체가 효과_규칙목록(요청.규칙 우선)으로 정확히
    # 계산되므로 이 guard도 같은 변수를 봐야 신규 규칙 케이스를 올바르게 통과시킨다. 공급가액·세액·
    # 합계 조정 로그와 "편집됨" 배지/부분취소 게이트(편집여부)는 원본목록 기준 그대로라 이 조건과
    # 무관하게 유지된다 — 실제 총액이 달라진 경우(예: 미분류 항목 누락)는 이 총계 레벨 신호로 계속
    # 포착된다.
    품목이력_항목 = _품목_변경_이력(기준목록, 최종목록) if (기준_대비_변경 and 효과_규칙목록) else []

    품목_삽입_sql = """
        INSERT INTO 거래명세서_품목
            (거래명세서번호, 구분, 순서, 코드, 품목, 작업명, 조, 조상단업무명, 구분표시, 규격, 비고, 수량, 단가, 금액, 구분자)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
    """
    # 조상단업무명(2026-08-30) — trim 후 빈 문자열 제외한 것만 유효(통합시트명 트리밍과 동일 패턴).
    조상단업무명맵_저장 = {
        조: 값.strip() for 조, 값 in (요청.조상단업무명맵 or {}).items() if (값 or "").strip()
    }
    ...
    try:
        with get_db() as conn:
            with conn.cursor() as cur:
                ...
                cur.execute("SELECT 역발행, 조별표지 FROM 거래처마스터 WHERE 거래처명=%s", (요청.거래처명,))
                _거래처설정행 = cur.fetchone()
                발행가능_초기값 = 0 if (_거래처설정행 and _거래처설정행["역발행"]) else 1
                # 조별표지(2026-08-30) — 이 거래처는 통합 명세서를 쓰지 않고 "요청내용" 표지에 조별
                # 블록을 나눠 보여주므로, 담당자가 실수로(또는 캐시 프리필로) 통합시트명을 입력해도
                # 저장 시점에 무조건 무효화한다(다운로드 시점에서만 막으면 DB에 값이 남아 재사용
                # 캐시(통합시트설정)에 다시 들어갈 위험이 있어, 애초에 저장 자체를 막는 쪽을 택함).
                if _거래처설정행 and _거래처설정행["조별표지"]:
                    통합시트명_저장 = None
                    상단업무명_저장 = None
                    if len(조집합) >= 2:
                        공급가액_직접입력_저장 = None
                ...
                if 조상단업무명맵_저장:
                    # "마지막 입력값" 재사용용 upsert(2026-08-30) — 통합시트설정과 동일 패턴, 조 단위로
                    # 한 단계 더 세분화. 실제 이번 확정에 등장한 조만 저장(맵에 있어도 조집합에 없는
                    # 이름은 무시 — 화면이 항상 현재 조_그룹목록 기준으로만 보내지만 방어적으로 필터).
                    업무명조합_현재 = billing.업무명조합_키(요청.업무명_목록 or [])
                    cur.executemany(
                        "INSERT INTO 조상단업무명설정 (거래처명, 업무명조합, 조, 조상단업무명) "
                        "VALUES (%s, %s, %s, %s) "
                        "ON DUPLICATE KEY UPDATE 조상단업무명=VALUES(조상단업무명)",
                        [
                            (요청.거래처명, 업무명조합_현재, 조, 값)
                            for 조, 값 in 조상단업무명맵_저장.items()
                            if 조 in 조집합
                        ],
                    )
                ...
                if 요청.품목_최종 is not None:
                    cur.executemany(품목_삽입_sql, [
                        (거래명세서번호, "원본", i, r["코드"], r.get("품목"), r.get("작업명"), None,
                         None, None, None, None, r["수량"], r["단가"], r["금액"], False)
                        for i, r in enumerate(원본목록)
                    ])
                    cur.executemany(품목_삽입_sql, [
                        (거래명세서번호, "최종", i, r["코드"] or None, r["표시품명"], None, r["조"],
                         조상단업무명맵_저장.get(r["조"]) if r.get("조") else None,
                    ...
                    cur.executemany(품목_삽입_sql, [
                        (거래명세서번호, "기준", i, r.get("코드") or None, r.get("표시품명"),
                         r.get("작업명"), r.get("조"), None, r.get("구분표시"), r.get("규격"), r.get("비고"),
                         r["수량"], r["단가"], r["금액"], False)
                        for i, r in enumerate(기준목록)
                    ])

                    # 품명 이력 갱신(2026-08-12 upsert 도입, 2026-08-29 "확정 시마다 덮어쓰기"로 변경)
                    # — 거래명세서_품목과 달리 취소(전체취소 시 DELETE FROM 거래명세서 CASCADE)해도
                    # 지워지지 않는 별도 테이블. 미리보기 "새 행 추가" 자동완성·"과거 품명 추가"
                    # (체크박스 일괄추가) 후보용. 조건식(규칙) 품명은 청구품목규칙으로 이미 재사용되므로
                    # 제외 — 수동입력=true인 행만("새 행 추가"로 직접 타이핑한 것만) 저장한다(사용자
                    # 요청, 2026-08-12). 품명별 조도 함께 저장(자동 반영 시 조까지 복원해달라는 요청).
                    #
                    # 2026-08-29: 예전엔 이번 확정에 수동입력 행이 하나도 없으면(전부 지우고 확정)
                    # INSERT 자체를 건너뛰어, 예전에 실수로 저장했던 품명이 계속 "가장 최근"으로 남아
                    # 영구히 자동 반영되는 문제가 있었다(사용자 제보 — 삼성화재해상보험 "내용증명" 오타
                    # 품명이 몇 주가 지나도 계속 재등장). 확정할 때마다 그 거래처의 이력을 통째로
                    # 지우고 이번 확정분으로만 다시 채우는 방식으로 변경 — 수동입력 0건이면 이력도
                    # 0건이 되어 다음 미리보기부터 자동 반영이 깨끗하게 사라진다(사용자 확정).
                    # 구분자 행(2026-09-01)은 제외 — 자료와 무관한 위치용 캡션이라 다음에 자동으로
                    # 다시 반영되면(항상 맨 아래에 붙는 이 기능의 특성상) 원래 위치와 안 맞게 엉뚱한
                    # 자리에 나타나는 문제가 있었음(사용자 제보). 필요하면 "구분자삽입" 버튼으로 그때
                    # 그때 원하는 위치에 다시 추가하면 된다.
                    품명별조 = {
                        r["표시품명"]: r.get("조")
                        for r in 최종목록
                        if r.get("표시품명") and r.get("수동입력") and not r.get("구분자")
                    }
                    cur.execute("DELETE FROM 거래명세서품명이력 WHERE 거래처명=%s", (요청.거래처명,))
                    if 품명별조:
                        cur.executemany(
