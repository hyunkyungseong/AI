...
export default function PricingMaterialSection({
  ...
  function openNew() {
    setEditing("new");
    // 무상차단코드(2026-08-25)에 걸린 항목을 기본값으로 잡으면, 아래 필터된코드옵션의 "지금 선택된
    // 값은 예외로 계속 보여준다" 규칙이 자기 자신을 예외 처리해버려 무상 체크가 있어도 그 항목이
    // 드롭다운에 계속 남는 버그가 있었다(2026-08-31, 사용자가 용지제작단가 무상 체크 상태에서 새
    // 자재단가를 추가하려는데 "출력자재비(용지)"가 여전히 보인다고 제보 — 하드코딩된 기본값
    // "출력자재비"가 하필 그 차단 대상 코드였음). 차단 안 된 첫 항목(대부분 "출력비", 자재비가
    // 아니라 서비스 비용이라 항상 차단 대상이 아님)을 기본값으로 골라 이 자기예외 루프를 피한다.
    set코드(코드옵션.find((c) => !무상차단코드?.has(c.value))?.value ?? "출력자재비");
    set단가("");
    set표시명("");
    set인쇄면("");
    set비고("");
